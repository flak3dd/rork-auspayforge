...
export function generateBankStatement(config: AppConfig, payslips: Payslip[]): BankStatement {
  ...
  const payDayOffset = payFrequency === 'weekly' ? 3 : 5;
  ...
  if (payslipPaymentDates.length > 0 && representativeNetPay > 0) {
    ...
    while (extraDate <= spanEnd) {
      ...
      if (!paymentDates.has(key)) {
        paymentDates.set(key, representativeNetPay);
        console.log('[BankStatement] Extended pay deposit forward on', key, 'for 
 ...
 + representativeNetPay.toFixed(2));
      }
      if (payFrequency === 'monthly') {
    ...
    while (backDate >= spanStart) {
      ...
      if (!paymentDates.has(key)) {
        paymentDates.set(key, representativeNetPay);
        console.log('[BankStatement] Extended pay deposit backward on', key, 'for 
 ...
 + representativeNetPay.toFixed(2));
  ...
  console.log('[BankStatement] Pay deposit dates within statement range:', Array.from(payslipDatesInRange).sort());
